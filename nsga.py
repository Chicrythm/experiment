...
import numpy as np
from scenario_train import Assignment
import matplotlib.pyplot as plt
import time
import xlsxwriter
startTime = time.time()
workbook = xlsxwriter.Workbook("scatter-60-20.xlsx")
...
## 评价函数
def ZDT3(ind):
    # 通过GMRA的指派结果得出的两个评价指标作为平衡值，一个是方差一个是总体表现
    f1, f2 = Assignment.RUN_GRA(ind[0], ind[1], ind[2])
    if (f1 < 5 or f1 < f2):
        return 100, -100
    return -f1, f2
