...
def KMV(Q, La, L, Lsum, dayFlights, index_sum, sum_ave):
    ...
    for i in range(0, row):
      for j in range(0, col):
        # 与 sum_ave 绝对差作为新的目标函数，避免非线性目标函数的出现
        all += abs(Q[i][j] - sum_ave) * lpvars[i][j]
...
def assignment(Q_Matrix, La, L, Lsum, dayFlights, index_sum, city, flights, sum_aver, devCoe, ratioCoe):
  ...
  # 用较优者作为提升和损失的参考基准，如improve_var为较优的方差值，因此作为分母
...
def GMRACF(Q_Matrix, La, L, Lsum, dayFlights, index_sum, flights):
  ...
  for i in range(len(Q_Matrix)):
    for j in range(len(Q_Matrix[0])):
      sumQmartix += Q_Matrix[i][j]
  # 矩阵Q的均值
  teamAver = sumQmartix / (len(Q_Matrix) * len(Q_Matrix[0]))


  TMatrix_GRA, result_GRA, performance_GRA = KM(Q_Matrix, La, L, Lsum, dayFlights, index_sum)
  initPerformance = performance_GRA
  lisGRA = []
  for i in range(len(TMatrix_GRA)):
    for j in range(len(TMatrix_GRA[0])):
  ...
  # 最优指派组均值
  bestTeam_aver = performance_GRA / flights
  init_sum = 0
  for i in range(len(lisGRA)):
...
def nsga(Q_Matrix, La, L, Lsum, dayFlights, index_sum, city, flights):
  ...
  for ind in front:
    # 0-基准值，1-偏离度，2-接受度，3-方差提升百分比，4-性能损失百分比
  ...
  tempLis = []
  for i in range(len(frontData)):
    # 大规模实验中只接受性能损失百分比为15%以下的个体
    if frontData[i][4] < 15:
      tempLis.append([(15 - frontData[i][4]), frontData[i][3], frontData[i][4]])
  # 以损失值最大的为排序基准，返回数据中即满足损失15%且方差提升最高的个体
  tempLis.sort()
...
if __name__ == '__main__':
  ...
  workbook = xlsxwriter.Workbook("uni50-60.xlsx")
  ...
  averIGMRACFImp = sumIGMRACFImp / (count - 1)
  averIGMRACFLoss = sumIGMRACFLoss / (count - 1)


  print('averFDImp', averFDImp)
  print('averFDLoss', averFDLoss)
  print('sumIGMRACFImp', averIGMRACFImp)
  print('sumIGMRACFLoss', averIGMRACFLoss)